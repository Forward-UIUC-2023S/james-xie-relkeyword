...
def extract_candidate_keywords(papers, seed_words):
    ...
    total_words = []
    vertex_weights = defaultdict(lambda: defaultdict(int))
    ...
    for word in candidate_keywords:
        if word in model.wv.key_to_index:
            for initial_word in seed_words:
                vertex_weights[word][initial_word] = model.wv.similarity(word, initial_word)
                co_occurence_similarity[word][initial_word] = co_occurence_similarity[word][initial_word]/doc_word_count[word]
    ...
    total_sum = 0
    for word in co_occurence_similarity:
        total_sum += sum(co_occurence_similarity[word].values())
        
    for word in candidate_keywords:
        # total_sum = sum(co_occurence_similarity[word][])
        for initial_word in seed_words:
            vertex_weights[word][initial_word] += domain_relevance[word]
            vertex_weights[word][initial_word] += co_occurence_similarity[word][initial_word]/total_sum
    return candidate_keywords, embedding_similarity, co_occurence, domain_relevance, co_occurence_similarity,doc_word_count, vertex_weights
...
def construct_graph(candidate_keywords, seed_words, edge_weights):
    ...
    for i in range(N):
        ...
        for j in range(M):
            ...
            if (edge_weights[word][initial_word] != 0):
                co_graph[i][j] = edge_weights[word][initial_word]
                
    vertex_weights = defaultdict(int)
    for word in candidate_keywords:
        for initial_word in seed_words:
            if initial_word in edge_weights[word]:
                vertex_weights[word] = 1
    ...
    return co_graph, indexed_array, vertex_weights
...
def main(papers, seed_words):
    candidate_keywords, embedding_similarity, co_occurence, domain_relevance, co_occurence_similarity, doc_word_count, edge_weights = extract_candidate_keywords(papers, seed_words)
    # with open('embedding_similarity.txt', 'w') as convert_file:
    #     convert_file.write(json.dumps(embedding_similarity))
        
    with open('domain_relevance_covid.txt', 'w') as convert_file:
        convert_file.write(json.dumps(domain_relevance))
        
    with open('co_occurrence_similarity_covid.txt', 'w') as convert_file:
        convert_file.write(json.dumps(co_occurence_similarity))
    
    with open('co_occurrence_covid.txt', 'w') as convert_file:
    ...
    co_graph, indexed_array, vertex_weights = construct_graph(candidate_keywords, seed_words, edge_weights)
    ...
    with open('results.txt_covid', 'w') as convert_file:
